...
public interface Translation<T1, T2> {
  ...
  Flux<WindowedValue<T2>> simple(Flux<WindowedValue<T1>> flux);
  ...
  class Identity<T> implements Translation<T, T> {
    ...
    @Override
    public Flux<WindowedValue<T>> simple(Flux<WindowedValue<T>> flux) {
      return flux;
    }

    @Override
    public Flux<? extends Flux<WindowedValue<T>>> parallel(
        Flux<? extends Flux<WindowedValue<T>>> flux, int parallelism, Scheduler scheduler) {
      return flux;
    }
  }
}
